...
def test_climatepy_filter(AOI):
    ...
    assert len(climatePy.climatepy_filter(asset='2019 Land Cover L48')) == 1

    # raise an error, id not in catalog
    with pytest.raises(Exception):
        climatePy.climatepy_filter(id="BLAH")

    # raise an error, asset not in catalog
    with pytest.raises(Exception):
        climatePy.climatepy_filter(id="MODIS", asset="BLAH")
    assert len(climatePy.climatepy_filter(id="maca_day", varname="pr", model=1, startDate="2000-01-01")) == 1
    
    # raise an error, model not in catalog
    with pytest.raises(Exception):
        climatePy.climatepy_filter(id="maca_day", varname="pr", model=1e9, startDate="2000-01-01")
    
    # raise an error, landcover not in US
    with pytest.raises(Exception):
        climatePy.climatepy_filter(AOI=make_aoi("egypt"), asset='2019 Land Cover L48')

    # correctly return a single asset
    assert len(climatePy.climatepy_filter(id="gridmet", varname="pr")) == 1

    # correctly return multiple assets
    assert len(climatePy.climatepy_filter(id="bcca", varname=['pr', 'tasmax', 'tasmin'],
                                ensemble='r1i1p1', model=["CSIRO-Mk3-6-0"],
                                scenario=['rcp45', 'rcp85'], startDate="2079-10-01")) == 6
    # raise an error, ensemble not in catalog
    with pytest.raises(Exception):
        climatePy.climatepy_filter(id="bcca", varname=['pr', 'tasmax', 'tasmin'],
